...
from risk_copilot.ingestion.github_client import GitHubRateLimitError
...
# Streamlit Community Cloud stores secrets in st.secrets rather than environment
# variables. Mirror an optional token into the environment so the shared GitHub
# client can authenticate without hard-coding credentials.
try:
    streamlit_github_token = st.secrets.get("GITHUB_TOKEN")
except Exception:
    streamlit_github_token = None
if streamlit_github_token and not os.getenv("GITHUB_TOKEN"):
    os.environ["GITHUB_TOKEN"] = str(streamlit_github_token)

service = RiskCopilotService(db_path=os.getenv("RISK_COPILOT_DB", "data/risk_copilot.db"))


@st.cache_data(ttl=300, show_spinner=False)
def cached_milestones(repo: str):
    return list_due_milestones(repo)


@st.cache_data(ttl=300, show_spinner=False)
def cached_milestone_inspection(repo: str, milestone_number: int):
    return inspect_milestone(repo, milestone_number)


def show_github_error(exc: Exception) -> None:
    if isinstance(exc, GitHubRateLimitError):
        reset = exc.reset_at
        if reset:
            st.warning(
                "GitHub's API allowance is temporarily exhausted. "
                f"The current reset time is {reset}. Cached data will still be reused when available."
            )
        else:
            st.warning(
                "GitHub's API allowance is temporarily exhausted. "
                "Cached data will still be reused when available."
            )
        if not os.getenv("GITHUB_TOKEN"):
            st.info(
                "For reliable public-repository assessments, add a GITHUB_TOKEN "
                "to the app's Streamlit Secrets. The app already supports it."
            )
    else:
...
def repository_page():
    ...
    if st.button("Load milestones"):
        try:
            ...
            st.session_state["repo_milestones"] = cached_milestones(normalized_repo)
            st.session_state["repo_name"] = normalized_repo
        except Exception as exc:
            show_github_error(exc)
    ...
    if milestones:
        ...
        if inspect_clicked:
            try:
                result = cached_milestone_inspection(
